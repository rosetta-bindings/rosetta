...
`jlcxx`, plays the same role pybind11 does for Python and N-API for Node. The binding is generated by rosetta's Julia backend and fully expanded — every field, method and constructor is an explicit `jlcxx` call (see `auto_jlcxx.cpp`).
