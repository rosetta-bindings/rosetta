// Copyright (c) fmaerten@gmail.com
// License: MIT

// Exercise the auto-generated bindings produced by auto_napi.cpp.
//
// Run after `npm install && npx cmake-js compile`.

'use strict';

const r = require('./reflected_person.node');

const p = new r.Person();
console.log('Person() created');

console.log('\n-- valid writes --');
p.name = 'Alice';
p.age = 42;
console.log(`  name=${p.name}, age=${p.age}`);

console.log('\n-- range violation (age=999) --');
try {
    p.age = 999;
} catch (e) {
    console.log(`  caught ${e.constructor.name}: ${e.message}`);
}

console.log('\n-- readonly violation (id=...) --');
try {
    p.id = 'p-999';
} catch (e) {
    console.log(`  caught ${e.constructor.name}: ${e.message}`);
}

console.log('\n-- method call --');
console.log(`  greet('Hello') -> ${p.greet('Hello')}`);

console.log('\n-- clear() --');
p.clear();
console.log(`  after clear: name=${p.name || '""'}, age=${p.age}`);
