...
|---|---|---|---|
| **Runtime visitor** | `rosetta/visitors/lua.h` | when the *generated* binding is compiled | **yes** — walks the class |
...
manifest.json ─(rosetta_gen)→ gen/ ─(build driver)→ ./generator ─(run)→ bindings/lua/
                                                       │                    auto_lua.cpp  ← includes visitors/lua.h
                                                       │                    CMakeLists.txt
...
## Layer 1 — the runtime visitor (`visitors/lua.h`)
...
[`include/rosetta/backends/`](../include/rosetta/backends/) (`python.h`, `node.h`,
`rest.h`, `wasm.h`) with its implementation alongside in
[`include/rosetta/backends/inline/`](../include/rosetta/backends/inline/)
(`python.hxx`, …) — are the reference for the CMake flags
(`-freflection -freflection-latest -fexperimental-library -fannotation-attributes`,
the clang-p2996 link line, etc.).

They live in **`rosetta::backend`** (`backend::Python`, `backend::Node`, …), so
the directory, the file and the namespace each say "backend" once and the class
says only which one. Your own backend does **not** belong in that namespace —
put it in yours, as the example above does; `rosetta::backend` is where
rosetta's own implementations live, not a registration point. What you inherit
from is `rosetta::Backend`, and what you call is the public API below.

The reflection-free header-only runtimes those backends emit `#include` lines
for — the N-API marshaller, the C#/Java C-ABI shims, the Qt and ImGui widget
kits — live under [`include/rosetta/runtime/`](../include/rosetta/runtime/)
(`node.h`, `csharp.h`, `java.h`, `qt_widgets.h`, `imgui.h`). They are compiled
by the *generated* project, never by the generator, which is why they are a
directory of their own rather than part of `backends/` or `visitors/`.
...
`CMakeLists.txt` ready to compile against your `visitors/lua.h`.
...
**IR accessors** (`<rosetta/generate.h>`) — how a backend spells a bound type:

- `std::string qualified_of(const GenClass&)` / `(const GenEnum&)` — the **C++** spelling (`arch::sinv::Data`), for anything the generated TU compiles
- `std::string exposed_of(const GenClass&)` / `(const GenEnum&)` — the **host-language** name (the manifest's `expose` override, else the identifier)
- `std::string exposed_object_of(const GenType&, const GenContext&)` — the exposed name of the type an IR entry *references*
- `bool names_type(const GenType&, const K&)` — does this IR type name that bound class / enum?

> These six lived in `rosetta::gen_detail` before 2026-08. That spelling still
> compiles — the old names are `using`-declarations onto these — but it was a
> "detail" namespace documented as an extension point, so they now sit in
> `rosetta::` with the rest of the backend API.
