...
// variant.
//
// Unlike Julia, which emits a thin `auto_jlcxx.cpp` that re-runs the
...
//
// Surface (mirrors the python feature set, spelled the Julia way):
//   - fields    -> getter `name(obj)` + mutating setter `name!(obj, v)`;
//                  `readonly` drops the setter; `range` validates on set;
//                  gated on the IR's copyability flags like python.
...
//   jlcxx equivalent).
//
// Registered under the "julia" target. Caveat: the generated file
...
// shared render helpers and after backends/python.h, whose
