...
// mode counterpart of the qt inspector.
//
...
// <rosetta/runtime/imgui.h> (the ImGui runtime/qt_widgets.h).
//
// Registered under the "imgui" target; builds with a stock C++17 compiler (the
// usual expanded caveat applies: bound headers must be stock C++, i.e.
// annotations supplied out of line). ROSETTA_IMGUI_FRAMES=N in the environment
// auto-exits after N frames (headless smoke tests / CI).
//
// Part of the generate pipeline (included by inline/generate.hxx after the qt
// backend, whose qtx_display()/qtx_method_ok() helpers it reuses). The emit()
// implementation lives in inline/imgui.hxx.
