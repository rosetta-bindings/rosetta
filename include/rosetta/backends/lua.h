...
//
// Registered under the "lua" target. Caveat: the generated file still
// `#include`s the bound headers, so a stock toolchain can build it only when
// *those headers are themselves stock C++* — i.e. annotations are supplied out
// of line (manifest "annotations": "...") rather than inline `[[=...]]`.
//
...
// reuses). The emit()/render() implementations live in inline/lua.hxx.
