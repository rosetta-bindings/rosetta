...
mode is a wheel-only switch (`-DROSETTA_STABLE_ABI=ON` to force it by hand).)MD";
        ...
        inline std::string nbx_method_segment(const GenClass &k, const GenMethod &m,
            ...
            // Same rule, same reason as python-expanded (px_ref_return): a
        ...
        inline std::string nbx_class(const GenClass &k, const GenContext &c) {
            ...
            if (!k.is_destructible) {
                std::fprintf(stderr,
                             ...
                             "(ref-counted?) — skipped\n",
                             "nanobind-expanded", k.name.c_str());
                return {};
            }
        ...
        inline std::string nanobind_expanded_source(const GenContext &c0) {
            // Same rule as python-expanded: where nanobind has the caster for
            ...
            // python-expanded backend gets for free, plus the smart pointers
        ...
        inline void NanobindExpandedBackend::emit(const GenContext &c) const {
            ...
            // section are shared verbatim with python-expanded (both come from
            // python_expanded_backend.hxx); only pyproject.toml and the ABI
