...
// after wasm_backend.h and after python_backend.h (whose qualify_std()
    ...
    template <typename F> F make_fn(emscripten::val f) { return fn_wrap<F>::make(f); }
        ...
        inline std::string wx_class(const GenClass &k, const GenContext &c) {
            ...
            for (const auto &m : k.methods) {
                ...
                if (!coverage::emit_overload(coverage::overloads::first_only, "wasm", k,
                                             m)) {
                    continue;
                }
                if (seq_touches(m)) {
                    ...
                    if (!wx_seq_eligible(m, c)) {
                        coverage::note_skip("wasm", k, m, "sequence_not_adaptable",
                    ...
                    segs.push_back((m.is_static ? ".class_function(\"" : ".function(\"") +
                                   m.name + "\", +[](" + decls + ") {\n" + body +
                                   "        })");
                    coverage::note_bound("wasm", k, m);
                    continue;
                }
                if (!wx_method_ok(m, c)) {
                    coverage::note_skip("wasm", k, m, "unmarshalable_signature",
                ...
                if (wx_params_have_callback(m.params)) {
                    ...
                    if (m.is_static) {
                        coverage::note_skip("wasm", k, m, "static_callback_receiver",
        ...
        inline std::string WasmBackend::render(const GenContext &c) const {
