...
// a "choices: …" tag. Included by inline/generate.hxx (via html_backend.h).
...
namespace rosetta {
  ...
  .sig { display: block; margin: .15rem 0; }
        ...
        inline std::string HtmlBackend::render(const GenContext &c) const {
        ...
        inline void HtmlBackend::emit(const GenContext &c) const {
    ...
    template <typename... Ts> inline std::string to_html(std::string lib) {
        return gen_detail::HtmlBackend{}.render(gen_detail::make_context<Ts...>(std::move(lib)));
    }
