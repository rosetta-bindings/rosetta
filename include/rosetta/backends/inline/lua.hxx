...
```
        ...
        inline std::string lua_class(const GenClass &k, const GenContext &c) {
            ...
            for (const auto &m : k.methods) {
                ...
                if (!coverage::emit_overload(coverage::overloads::first_only, "lua", k, m)) {
                    continue;
                }
                if (seq_touches(m)) {
                    ...
                    if (lx_seq_eligible(m, c)) {
                        s += lx_seq_method(k, m);
                        coverage::note_bound("lua", k, m);
                    } else {
                        coverage::note_skip("lua", k, m, "sequence_not_adaptable",
                ...
                if (!lx_method_ok(m, c)) {
                    coverage::note_skip("lua", k, m, "unmarshalable_signature",
        ...
        inline std::string Lua::render(const GenContext &c) const {
            return lua_expanded_source(c);
        }

        inline void Lua::emit(const GenContext &c) const {
            auto dir = c.out_dir / "lua";
            ...
            write_file(dir / "README.md",
                       readme("lua", c, subst(LUAX_README_BUILD, {{"LIB", c.lib}})));
        }
