...
%  Section 10 reports E1-E4, all measured; E5 is a scaffold.
%  Sections 11 (case study), 13 (related work) and 14 remain to write.
...
two thirds of the interface recovered by deferring resolution alone.
\todo{extend this headline with the E5 real-library coverage figure}
\end{abstract}
...
Table~\ref{tab:coverage} reports the coverage of the examples shipped with the
system. \todo{replace with E5, a real third-party library; keep this as the
smaller worked table.}
...
Java backends express two members of eight: their marshalling goes through JSON
and a name-keyed operation table, and six members do not survive that. A
generator that reported only its successes would show the same green as Python's
8-of-8. Making the failure legible, per target and per member, is what turns
``multi-target'' from a claim into a measurement --- and it is what lets a user
choose a target with knowledge of what they are giving up.

% =====================================================================
\section{Experimental evaluation}
\label{sec:eval}

\gap{E1--E4 are run and reported below; E5 is a scaffold. The harness lives in
...
\todo{An unmodified third-party scientific C++ library. Bound/skipped per target
with a reason histogram. Answers: what fraction of a realistic C++ API can be
exposed automatically, and why does the remainder fail?}

% =====================================================================
\section{Case study: one geophysical library, four interfaces}
\label{sec:case}

\gap{Scaffold --- E6.}

\todo{A \code{Mesh}/\code{Field}/\code{Solver} library driven from Python
(analysis), Julia (numerics), Qt (an interactive property editor generated by
query over \code{fields()}) and WebAssembly (a browser demo). Then evolve the
library and show the interface acquiring the new properties with no change to
any binding --- the concrete payoff of Section~\ref{sec:dynamic}.}
